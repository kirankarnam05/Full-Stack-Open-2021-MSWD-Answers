...
import {
	...
	withRouter, Redirect, Route, Link
...
const Anecdote = ({ anecdote }) => {
	...
	return (
		<div>
			...
			<div style = { spacing } > has { anecdote.votes } votes </div>
			<div style = { spacing } >
...
const CreateNewHistory = (props) => {
	...
	const handleSubmit = (e) => {
		...
		props.history.push('/')
		props.notificationHandler(`a new anecdote ${content}`)
	}
		...
		<div>
			<Router>
				<div>
					...
					<Menu />
					{ notification === '' ? <div></div> : <div style = { notificationStyle } > {notification} </div> }
					<Route exact path = '/' render = { () => <AnecdoteList anecdotes = { anecdotes } /> } />
					<Route exact path = '/anecdotes/:id' render = 
						{ ({ match }) => <Anecdote anecdote = { anecdoteById(match.params.id) } /> } 
					/>
					<Route path = '/create' render = 
						{ () => <CreateNew addNew = { addNew } notificationHandler = { notificationHandler } /> } 
					/>
